...
let drawCard = (cards:Array<number>) =>{
    //拷贝一份数组
    let randomCards:Array<number> = Object.assign({},cards)
    //打乱数组顺序
...
let owner:string = 'U001' //地主用户
user[owner].push(...drawCardResult[3])
user[owner] = user[owner].sort((a:number,b:number)=>b-a)
console.log('用户手牌：',JSON.stringify(user))


//出单张牌判断
let C1 = (uid:string,...ns:number[])=>{
